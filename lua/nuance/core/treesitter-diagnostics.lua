...
vim.api.nvim_create_autocmd({ 'FileType', 'TextChanged', 'InsertLeave' }, {
  ...
  pattern = { 'lua', 'bash', 'vim', 'c' },
  group = vim.api.nvim_create_augroup('editor.treesitter', { clear = true }),
  --- @param args vim.api.keyset.create_autocmd.callback_args
  callback = vim.schedule_wrap(function(args)
    if not vim.g.treesitter_diagnostics then
      vim.diagnostic.clear(namespace, args.buf)
      return
    end
