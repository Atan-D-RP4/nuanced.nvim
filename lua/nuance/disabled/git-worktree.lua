return {
  {
    ...
    config = function()
          ...
          for line in output:gmatch("[^\r\n]+") do
              ...
              if fields[2] ~= "(bare)" then
                  table.insert(results, {
                      ...
                      sha = fields[2],
                      branch = fields[3],
                      display = string.format("%-20s %-40s %s",
                          fields[3]:gsub("[%[%]]", ""),
                          path.normalize(fields[1]),
                          fields[2])
                  })
              end
          end
      ...
      local function delete_worktree(selected)
          ...
          local confirm_msg = force_next_deletion
              and "Force delete worktree " .. worktree.path .. "? [y/N] "
          ...
          fzf.fzf_exec(displays, {
              ...
              actions = {
                  ...
                  ["ctrl-f"] = function()
                      ...
                      local msg = force_next_deletion
                          and "The next deletion will be forced"
                          or "The next deletion will not be forced"
                      print(msg)
                  end
              }
          })
